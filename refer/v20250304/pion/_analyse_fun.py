...
class data_analyse:
    def __init__(
        ...
        self.Nx = Nx
        self.Nt = Nt
        self.N_ENV = ENV.shape[0], self.ENV = ENV
        self.N_P = P.shape[0], self.P = P
        self.N_data = filepath.shape[0], self.filepath = filepath
        self.link_max = link_max
        self.N_link = 2*link_max+1
        self.N_tsep = tsep.shape[0], self.tsep = tsep
        self.conf = np.asarray(range(N_start, N_start+gap*Ncnfg+1, gap))
...
def read_iog(filepath, Nx, Nt, P, ENV, N_start, gap, Ncnfg, tsep_array, link_max, type):
    ...
    if type == '3pt' or type == 'ratio':
        for i in range((N_data-1)*N_tsep*N_P*Ncnfg):
            tsep_indx = (i)%N_tsep
            conf_indx = (i//(N_tsep))%Ncnfg
            P_indx = (i//(N_tsep*Ncnfg))%N_P
            data_indx = (i//(N_tsep*Ncnfg*N_P))%N_data
            data = iog_read(filepath[data_indx]%(Nx, Nt, P[P_indx,0], P[P_indx,1], P[P_indx,2], -1, conf_array[conf_indx], tsep_array[tsep_indx], link_max), intrptr)
            data_readed_3pt[data_indx,P_indx,0,tsep_indx,:,conf_indx,:,0] = np.append(data['Re'].to_numpy().reshape(N_link,16,Nt)[link_max+1:,8][::-1],data['Re'].to_numpy().reshape(N_link,16,Nt)[:link_max+1,8],axis=0)
            data_readed_3pt[data_indx,P_indx,0,tsep_indx,:,conf_indx,:,1] = np.append(data['Im'].to_numpy().reshape(N_link,16,Nt)[link_max+1:,8][::-1],data['Im'].to_numpy().reshape(N_link,16,Nt)[:link_max+1,8],axis=0)
    if type == '2pt' or type == 'ratio':
        for i in range(1*N_P*Ncnfg):
            conf_indx = (i)%Ncnfg
            P_indx = (i//(Ncnfg))%N_P
            data = iog_read(filepath[-1]%(Nx, Nt, P[P_indx,0], P[P_indx,1], P[P_indx,2], -1, conf_array[conf_indx]), intrptr)
...
def jackknife_ctr_err(data): # data (Ncnfg, Nt)
    ...
    return jcknf_data
def meff_2pt(data,dtype,time_fold): # data:1, P, ENV, 1, 1, Ncnfg, Nt, dtype=complex
    Ncnfg = data.shape[-2]
    Nt = data.shape[-1]
    if time_fold == 1 :
        T_hlf = (Nt-1)
    ...
    if (dtype=='log'):
        ...
        if(dtype=='cosh'):
            data_mean = (np.array(np.arccosh((data_mean_ini[...,2:] + data_mean_ini[:-2])/(2 * data_mean_ini[...,1:-1]))))
            data_cosh_sample = (np.array(np.arccosh((data_sample_ini[...,2:] + data_sample_ini[...,:-2])/(2 * data_sample_ini[...,1:-1]))))
...
def PDF_3pt_2pt(data,tsep_array): # data, P, ENV, tsep, link, Ncnfg, Nt, dtype=complex
    ...
    Ncnfg = data.shape[5]
    Nt = data.shape[6]
    PDF_3pt_2pt_mean = np.zeros((N_data-1, N_P, N_ENV, N_tsep, N_link, Nt),dtype=np.double)
    PDF_3pt_2pt_err = np.zeros((N_data-1, N_P, N_ENV, N_tsep, N_link, Nt),dtype=np.double)
    PDF_3pt_2pt_cov = np.zeros((N_data-1, N_P, N_ENV, N_tsep, N_link, Nt, Nt), dtype=np.double)
    ...
    return PDF_3pt_2pt_mean, PDF_3pt_2pt_err, PDF_3pt_2pt_cov
...
def min_fun_descent(function, X, X0):
    ...
    for n in range(N):
        ...
        dd_fun[n] = sp.diff(function, X[n], 2) 
        d_fun_value[n] = (float)(d_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf())  
        dd_fun_value[n] = (float)(dd_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf())  
    function_value = 0
    while (cycle_index <= max_cycle_index and sum(abs(d_value) for d_value in d_fun_value) >= delta):
        
        cycle_index += 1
        for n in range(N):
            X0[n] = X0[n] - d_fun_value[n] * alpha# * (abs(d_fun_value_2)/abs(dd_fun_value_2)) # * np.exp(abs(d_fun_value_2)-abs(dd_fun_value_2)) # * u
            d_fun_value[n] = (float)(d_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf()) 
            dd_fun_value[n] = (float)(dd_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf())  
        ...
        print('dd_fun_value:',dd_fun_value)
