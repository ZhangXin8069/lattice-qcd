...
class data_analyse:
    ...
    def read_iog_3pt(self, filepath):
        ...
        if self.time_fold == 1:
            readed = readed[...,:self.Nt//2+1,:] - readed[...,self.Nt//2-1:,:][...,::-1,:]
        self.readed['readed_3pt_iog'] = readed  
    ...
    def read_data_3pt(self,filepath):
        ...
        if self.time_fold == 1:
            readed = readed[...,:self.Nt//2+1,:] - readed[...,self.Nt//2-1:,:][...,::-1,:]
        self.readed['readed_3pt_data'] = readed[...,1:]
    ...
    def PDF(self, data_type): # data, P, ENV, tsep, link, Ncnfg, Nt, dtype=complex
        ...
        PDF_3pt_2pt_mean = np.zeros((self.N_P, N_ENV, self.N_tsep, self.N_link, self.Nt),dtype=np.double)
        PDF_3pt_2pt_sample = np.zeros(( self.N_P, N_ENV, self.N_tsep, self.N_link, Ncnfg, self.Nt))
        PDF_3pt_2pt_cov = np.zeros((self.N_P, N_ENV, self.N_tsep, self.N_link, self.Nt, self.Nt), dtype=np.double)
        # the 3pt/2pt part
        for tsep_indx in range(self.N_tsep):
            PDF_3pt_2pt_sample[:,:,tsep_indx,:,:,:self.tsep[tsep_indx]+1] = \
                self.readed_jcknf['readed_3pt_%s'%(data_type)][...,tsep_indx,:,:,:self.tsep[tsep_indx]+1] \
                / self.readed_jcknf['readed_2pt_%s'%(data_type)][...,self.tsep[tsep_indx]].reshape(self.N_P, N_ENV, 1, Ncnfg, 1)
            PDF_3pt_2pt_mean[:,:,tsep_indx,:,:self.tsep[tsep_indx]+1] = \
                self.readed_mean['readed_3pt_%s'%(data_type)][...,tsep_indx,:,:self.tsep[tsep_indx]+1] \
                / self.readed_mean['readed_2pt_%s'%(data_type)][...,self.tsep[tsep_indx]].reshape(self.N_P, N_ENV, 1, 1)
            #ratio cov
        ...
        #ratio_err
        self.PDF_mean['PDF_%s'%(data_type)] = PDF_3pt_2pt_mean
        self.PDF_err['PDF_%s'%(data_type)] = np.std(PDF_3pt_2pt_sample, axis = -2)*np.sqrt(Ncnfg-1)
        self.PDF_cov['PDF_%s'%(data_type)] = PDF_3pt_2pt_cov
        # PDF_3pt_2pt_data = np.array([PDF_3pt_2pt_mean,PDF_3pt_2pt_err]) #size = (2,data_1-1,t_sep+1)
    def link_analyse(self, data_type):
        if data_type == 'iog':
            N_ENV = 1
        elif data_type == 'data':
            N_ENV = self.N_ENV
        ...
        for tsep_indx in range(self.N_tsep):
            link_y[...,tsep_indx,:] = np.mean(self.PDF_mean['PDF_%s'%(data_type)][:,:,tsep_indx,:,self.tsep[tsep_indx]//2-5:self.tsep[tsep_indx]//2+5], axis=-1)
            link_err[...,tsep_indx,:] = np.std(self.PDF_mean['PDF_%s'%(data_type)][:,:,tsep_indx,:,self.tsep[tsep_indx]//2-5:self.tsep[tsep_indx]//2+5], axis=-1)
            
        self.link_y['link_%s'%(data_type)] = link_y
        self.link_err['link_%s'%(data_type)] = link_err
    def plot_meff_2pt(self, data_type, y_range):
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        # ax.set_title('%s_meff_%dx%d_%s_%s'%(self.hadron,self.Nx,self.Nt,self.meff_type,data_type),fontdict={'fontsize':30,'fontweight':'light'})
        ax.set_ylim(y_range)
        ax.set_xlabel('%s'%('t/a'))
        ...
        for P_indx in range(self.N_P):
            N_range = self.meff_data_2pt['meff_2pt_%s_mean'%(data_type)][P_indx, -1].shape[0] - self.Nt_con//2
            ax.errorbar(np.arange(N_range), self.meff_data_2pt['meff_2pt_%s_mean'%(data_type)][P_indx, -1, :N_range], yerr=self.meff_data_2pt['meff_2pt_%s_err'%(data_type)][P_indx, -1, :N_range], alpha=0.5, marker = self.marker_array[P_indx], capsize=3.5, capthick=1.5, label='P=(%d,%d,%d)'%(self.P[P_indx,0],self.P[P_indx,1],self.P[P_indx,2]), linestyle='none',elinewidth=2) # fmt = 'bs'
        plt.legend(fontsize=18)
        fig.savefig("%s/%s"%(self.save_path, '%s_meff_%dx%d_%s_%s.pdf'%(self.hadron,self.Nx,self.Nt,self.meff_type,data_type)))
    def plot_meff_ENV(self, y_range):
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ...
        plt.legend()
        fig.savefig("%s/%s"%(self.save_path, '%s_meff_%dx%d_%s_ENV.pdf'%(self.hadron, self.Nx, self.Nt, self.meff_type)))
    
    def plot_C2pt_ENV(self, y_range):
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ...
        for tsep_indx in range(self.N_tsep):
            ...
            plt.legend()
        fig.savefig("%s/%s"%(self.save_path, '%s_C2pt_%dx%d_ENV'%(self.hadron, self.Nx, self.Nt)))
    
    def plot_PDF_ENV(self, flavour, y_range):
        
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ax.set_title('PDF_ENV_%s'%(flavour),fontdict={'fontsize':30,'fontweight':'light'})
        ax.set_ylim(y_range)
        ax.set_xlabel('%s'%('N_ENV'))
        ax.set_ylabel('%s'%('$C_{\mathrm{3pt}}$/$C_{\mathrm{2pt}}$')) # $E_{\mathrm{2pt}}$/Gev
        for i in range(len(self.link_y.keys())):
            name = str(list(self.link_y.keys())[i])
            if name == 'link_iog': N_ENV_change = 450; label = 'chroma'
            elif name == 'link_data': N_ENV_change = self.ENV; label = 'distillation'
            for tsep_indx in range(self.N_tsep):
                ax.errorbar(N_ENV_change, self.link_y[name][0,:,tsep_indx,self.link_max], yerr=self.link_err[name][0,:,tsep_indx,self.link_max], alpha=0.5, marker = self.marker_array[tsep_indx], capsize=3.5, capthick=1.5, label='%s_tsep%d'%(label,self.tsep[tsep_indx]), linestyle='none',elinewidth=2) # fmt = 'bs'
                plt.legend(loc=2)
        fig.savefig("%s/%s"%(self.save_path, '%s_PDF_%dx%d_ENV_%s.pdf'%(self.hadron, self.Nx, self.Nt, flavour)))
    
    def plot_link_C3pt_C3pt(self, y_range):
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ax.set_title('link_C3/C3',fontdict={'fontsize':30,'fontweight':'light'})
        ax.set_ylim(y_range)
        ...
        for i in range(len(self.link_y.keys())):
            name = str(list(self.link_y.keys())[i])
            if name == 'link_iog': N_ENV_change = range(1); ENV_array_change=np.asarray([-1]); data_type = 'chroma'; N_ENV = 1
            elif name == 'link_data': N_ENV_change = range(-3,0,1); ENV_array_change = self.ENV; data_type = 'distillation'; N_ENV = self.N_ENV
            link_y_change = self.link_y[name] / self.link_y[name][...,self.link_max].reshape(self.N_P,N_ENV,self.N_tsep,1)
            link_err_change = self.link_err[name] / self.link_y[name][...,self.link_max].reshape(self.N_P,N_ENV,self.N_tsep,1)
            for ENV_indx in N_ENV_change:
                ax.errorbar(self.link, link_y_change[0,ENV_indx,0], yerr=link_err_change[0,ENV_indx,0], alpha=0.5, marker = self.marker_array[ENV_indx], capsize=3.5, capthick=1.5, label='%s-ENV%d'%(data_type,ENV_array_change[ENV_indx]),linestyle='none',elinewidth=2) # fmt = 'bs'
                plt.legend()
                    
        fig.savefig("%s/%s"%(self.save_path, '%s_link_%dx%d_C3_C3.pdf'%(self.hadron, self.Nx, self.Nt)))
        
    def plot_link_ratio(self, y_range):
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ax.set_title('link_ratio',fontdict={'fontsize':30,'fontweight':'light'})
        ax.set_ylim(y_range)
        ax.set_xlabel('%s'%('link_indx'))
        ...
        for i in range(len(self.link_y.keys())):
            name = str(list(self.link_y.keys())[i])
            link_y_change = self.link_y[name]
            link_err_change = self.link_err[name]
            if name == 'link_iog': N_ENV_change = range(1); ENV_array_change=np.asarray([-1]); data_type = 'chroma'
            elif name == 'link_data': N_ENV_change = range(-3,0,1); ENV_array_change = self.ENV; data_type = 'distillation'
            for ENV_indx in N_ENV_change:
                ax.errorbar(self.link, link_y_change[0,ENV_indx,0], yerr=link_err_change[0,ENV_indx,0], alpha=0.5, marker = self.marker_array[ENV_indx], capsize=3.5, capthick=1.5, label='%s-ENV%d'%(data_type,ENV_array_change[ENV_indx]),linestyle='none',elinewidth=2) # fmt = 'bs'
                plt.legend()
                    
        fig.savefig("%s/%s"%(self.save_path, '%s_link_%dx%d_ratio.pdf'%(self.hadron, self.Nx, self.Nt)))
        
    def plot_link_indx_ENV(self, indx, y_range, normal:int=0):
        
        plt.rcParams.update({'font.size':25})
        fig, ax = plt.subplots(1,1, figsize=(20, 20*0.5))
        fig.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        ax.set_title('link_indx%d_ratio'%(indx),fontdict={'fontsize':30,'fontweight':'light'})
        ax.set_ylim(y_range)
        ax.set_xlabel('%s'%('link_indx'))
        ax.set_ylabel('%s'%('$C_{\mathrm{3pt}}$ / $C_{\mathrm{2pt}}$')) # $E_{\mathrm{2pt}}$/Gev # (N_data-1, N_P, N_ENV, N_tsep, N_link, inp.Nt) (N_data-1, N_P, N_ENV, N_tsep, N_link)
        for i in range(len(self.link_y.keys())):
            name = str(list(self.link_y.keys())[i])
            if name == 'link_iog': N_ENV_change = self.ENV[-1]; data_type = 'chroma';  N_ENV = 1
            elif name == 'link_data': N_ENV_change = self.ENV; data_type = 'distillation'; N_ENV = self.N_ENV
            if normal == 1:
                link_y_change = self.link_y[name] / self.link_y[name][...,self.link_max].reshape(self.N_P,N_ENV,self.N_tsep,1)
                link_err_change = self.link_err[name] / self.link_y[name][...,self.link_max].reshape(self.N_P,N_ENV,self.N_tsep,1)
            else:
                link_y_change = self.link_y[name]
                link_err_change = self.link_err[name]
            ax.errorbar(N_ENV_change, link_y_change[0,:,0, indx+self.link_max], yerr=link_err_change[0,:,0, indx+self.link_max], alpha=0.5, marker = self.marker_array[i], capsize=3.5, capthick=1.5, label='%s_indx%d'%(data_type, indx),linestyle='none',elinewidth=2) # fmt = 'bs'
            plt.legend()
                    
        fig.savefig("%s/%s"%(self.save_path, '%s_link_indx%d_%dx%d_ratio.pdf'%(self.hadron, indx, self.Nx, self.Nt)))
        
def min_fun_descent(function, X, X0):
    alpha = 1e-15
    br = sp.symbols('br')
    delta = 1e-12
    ...
    d_fun_value = [None] * N
    for n in range(N):
        d_fun[n] = sp.diff(function, X[n]) 
    X0_dic = sp.solve([d_fun[n] for n in range(N)],[X[n]for n in range(N)])
    X0 = [X0_dic[indx] for indx in X]
    d_fun_value = [(float)(d_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf())  for n in range(N)]
    while (cycle_index <= max_cycle_index and sum(abs(d_value) for d_value in d_fun_value ) >= delta):
        ...
        X0 = [X0[i] - d_fun_value[i] * beta * alpha for i in range(N)]
        print('beta:',beta)
        d_fun_value = [(float)(d_fun[n].subs({X[i]: X0[i] for i in range(N)}).evalf()) for n in range(N)]
