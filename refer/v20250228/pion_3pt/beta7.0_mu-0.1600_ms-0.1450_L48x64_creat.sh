...
#SBATCH -w gpu042
#SBATCH --output=beta7.0_mu-0.1600_ms-0.1450_L48x64_creat.out
#SBATCH --array=0-75
#SBATCH --nodes=1
##SBATCH -n 
...
xml=./xml/${conf_type}/${hadron}_2pt_tsep${tsep}_${conf}.xml
python ${exe} ${hadron} ${conf} ${tsep} ${mass} > ${xml}
export QUDA_RESOURCE_PATH=./xml/${conf_type}/mass&{mass}
QUDA_ENABLE_TUNING=1 \
mpirun -x QUDA_RESOURCE_PATH -n 4 ${chroma} -geom 1 1 2 2 -i ${xml} > ./output/${conf_type}_${conf} 2>&1  
