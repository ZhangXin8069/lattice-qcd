...
namespace Chroma
{
  ...
  void InlineMyMeasIOG::func( unsigned long update_no, XMLWriter& xml_out)
  {
    ...
    for ( int i = 0; i < operator_no; i++ )
    {
      ...
      {
        corr = LatticeComplex( trace(T_unpol * traceColor(L_prop * traceSpin(quarkContract13(L_prop * Cg5, Cg5 * L_prop))))
                + trace(T_unpol * traceColor(transposeSpin(quarkContract24(Cg5 * L_prop, L_prop * Cg5)) * L_prop)) );
      }
      else
      {
